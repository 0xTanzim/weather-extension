...
// Wrap content script in IIFE for graceful early exit
(function () {
  ...
  if (
    ...
    console.log(
      'Weather extension: Content script not allowed on this URL type - gracefully exiting'
    );
    // Gracefully exit without throwing error - this is the best practice
    // The content script will simply not run on restricted pages
    return;
  }
  ...
  function stopDrag() {
    ...
    isDragging = false;
    if (overlayElement) {
      const header = overlayElement.querySelector(
        '.drag-handle'
      ) as HTMLElement;
      if (header) {
